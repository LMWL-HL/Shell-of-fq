...
def add_usr(n):
    ...
    print("\tlevel:\0" + str(load_dict["inbounds"][0]["settings"]["clients"][0]["level"]))
    print("\talterId:\0" + str(load_dict["inbounds"][0]["settings"]["clients"][0]["alterId"]))

def remove_user(dic):
    load_f = open("/etc/v2ray/config.json","r",encoding="utf-8")
    load_dict = json.load(load_f)
    if int(sorted(dic)[-1])-1 <= load_dict["inbounds"][0]["settings"]["clients"]:
        load_f.close()
    else:
        return "超出用户数量范围！"
    for i in dic:
        print("已删除用户：")
        print("\tid:\0" + str(load_dict["inbounds"][0]["settings"]["clients"][int(i)-1]["id"]))
        load_f = open("/etc/v2ray/config.json","r",encoding="utf-8")
        load_dict = json.load(load_f)
        del load_dict["inbounds"][0]["settings"]["clients"][int(i)-1]
        load_f.close()
